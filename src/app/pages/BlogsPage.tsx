...
export function BlogsPage() {
  ...
  const [categories, setCategories] = useState<string[]>(['All', 'Budget Travel tips', 'Backpacking tips', 'Travel Tips', 'Visa and Documentation Guides', 'Hotel & Accomodation Reviews', 'Destination Guides', 'HIdden / Offbeat Travel', 'Seasonal Travel', 'Mobile PHotography Tips']);
  ...
  const loadBlogs = async () => {
    try {
      ...
      const formattedPosts = data.map(post => ({
        ...
        visitedLocation: 'Location', // Default fallback
      ...
      const usedCategories = new Set(formattedPosts.map(p => p.category));
      // Merge with defaults
      const defaultCats = ['Budget Travel tips', 'Backpacking tips', 'Travel Tips', 'Visa and Documentation Guides', 'Hotel & Accomodation Reviews', 'Destination Guides', 'HIdden / Offbeat Travel', 'Seasonal Travel', 'Mobile PHotography Tips'];
  ...
  const filteredPosts = posts.filter(post => {
    ...
    // Handle 'all' category and specific category selection
    const matchesCategory = selectedCategory === 'all' || post.category.toLowerCase() === selectedCategory.toLowerCase();

    return matchesSearch && matchesCategory;
  });

  // Magazine layout - first filtered post as hero, then grid
  // If featured post exists in filtered results, use that as hero, otherwise first one
  let heroPost = filteredPosts.find(p => p.isFeatured);
  if (!heroPost && filteredPosts.length > 0) {
    heroPost = filteredPosts[0];
  }

  // Filter out the hero post from the rest
  const restPosts = filteredPosts.filter(p => p.id !== heroPost?.id);
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Stories, Tips & Guides • {posts.length} Articles
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <div className="relative mx-auto max-w-2xl">
            ...
            <input
              type="text"
              placeholder="Search stories, tips, or guides..."
              value={searchTerm}
          ...
          <div className="flex gap-1 overflow-x-auto py-3">
            {categories.map((category) => (
              <button
                ...
                className={`whitespace-nowrap border-b-2 px-4 py-2 font-['Inter'] text-xs font-bold uppercase tracking-wider transition-all ${(selectedCategory === 'all' && category === 'All') || selectedCategory.toLowerCase() === category.toLowerCase()
